...
I recruited 27 participants in total on SONA for the formal experiment. After excluding participants whose overall accuracy is lower than 60%, I was left with 21 participants (mean age = 19.9, range = 18, 24) in the final data analysis. I also excluded trials with unreasonable response time. 

#### Differences from pre-data collection methods plan

Added exclusion criteria: participants are excluded if their mean accuracy across all trials < 60%; specific trials are excluded if response time outside of 50-2000 ms. Resized images to bigger than original study to control for smaller screen sizes.
...
plot_psychometric_participant <- function(df, pid, extreme_thresh = 0.15) {
  ...
  # ========== FIT ONE FULL MODEL (like the paper does) ==========
  model_full <- glm(
    chose_right ~ log_num_ratio + log_size_ratio + log_spacing_ratio,
    data = d,
    family = binomial(link = "probit")
  ...
  # ========== GET TYPICAL EXTREME VALUES ==========
  
  # For congruent trials, Size/Spacing have SAME SIGN as numerosity
  # For incongruent trials, Size/Spacing have OPPOSITE SIGN as numerosity
  ...
  # Calculate typical extreme values (use median for robustness)
  # POSITIVE values for congruent (same direction)
  # NEGATIVE values for incongruent (opposite direction)
  ...
  # BLACK LINE: All trials (Size=0, Spacing=0, full range)
  pred_all <- data.frame(
  ...
  # RED DASHED: Size congruent
  # When num_ratio > 0: size_ratio > 0 (congruent)
  # When num_ratio < 0: size_ratio < 0 (congruent)
...
The expploratory analyses I conducted was on the coefficient of each dimension. From the individual GLMs, I hope to test whether all numerosity/spacing/size coefficients are significantly different from zero. The result shows that both numerosity and spacing had a large effect, whereas size has a smaller efffect. 
...
This replication successfully confirmed the primary finding of Testolin et al. (2020): human adults use both numerical and non-numerical visual features when making rapid numerosity judgments. Our confirmatory analysis using individual-level GLM fits revealed significant effects of numerosity (mean β = 3.68, t(22) = 24.12, p < 0.001) and spacing (mean β = 1.09, t(22) = 7.84, p < 0.001), but not size (mean β = 0.19, t(22) = 1.53, p = 0.14). These results closely parallel the original study's findings (numerosity: t(39) = 23.54; spacing: t(39) = 7.21; size: t(39) = 1.37), with nearly identical effect sizes and the same pattern of statistical significance. The group-level GLMM analysis corroborated these individual-level results, showing robust main effects of numerosity (β = 3.71, z = 28.45, p < 0.001) and spacing (β = 1.12, z = 12.89, p < 0.001), with no significant effect of size (β = 0.18, z = 1.42, p = 0.16). We therefore conclude that this is a successful replication of the original finding.
